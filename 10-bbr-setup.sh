...
# (ssh public key)
om -t https://${OPSMANAGER} -u "${ADMIN}" -p "${OPSMANAGERPWD}" -k curl -p /api/v0/deployed/director/credentials/bbr_ssh_credentials -s | jq -r '.[] | .value.public_key_pem' > bbr_key.pem
